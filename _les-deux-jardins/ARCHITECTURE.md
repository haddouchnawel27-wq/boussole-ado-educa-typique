...
2. **Cap Educa / Boussole** (racine) — *audit Cowork : rien de cassé techniquement, le souci est la conception (mélange des publics).*
   ...
   → **Option A (retenue, la + simple + sûre)** : Boussole = boîte **ADO/FAMILLE pure**. **Vue Ado par défaut** ✅ **FAIT le 16/07** (`assets/js/app.js` : `publicEffectif()` défaut « ado » au lieu de « pro » → un ado n'atterrit plus sur « tout voir » ; les 4 outils pro sont déjà masqués hors mode pro). **Reste** : l'option « Praticienne · tout voir » est **encore sélectionnable** (gardée pour que Nawel accède à ses 4 outils tant que leurs versions Cockpit n'existent pas). À trancher avec Nawel : **code léger** sur cette option **OU** migrer les 4 outils (Fiches jeunes · Journal ABC · Dossier · Modèles anamnèse/PAI) vers le **Cockpit** (vrai compte) puis retirer l'option pro.

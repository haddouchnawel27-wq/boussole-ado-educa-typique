...
- **✅ Fusion « Sous le capot » FAITE (7 juil)** : l'artifact de Design/Cowork est intégré en `/chef-chantier/sous-le-capot.html` (liens croisés : bouton « Le grand contrôle » dans le garage ↔ « Passer au chantier » → `index.html`). Corrections apportées : prénom du jeune retiré du code (défaut vide + placeholder ; il tape son prénom lui-même, stocké en local), prénom du binôme adulte retiré, lien cassé `chef-de-chantier.html` → `index.html`. **IDs de missions harmonisés avec la version Cowork** : `m0` = Sous le capot, `m7` = frein compter 10, `m13` = procédure zone rouge (l'ancien `m7` de la v2). Clés localStorage séparées : `chantier` + `educatypique-souslecapot-state`.
- **🌐 Netlify (déployé par Nawel)** : `chef-de-chantier-ethan-apli.netlify.app` — bugs repérés : lien « Passer au chantier » → 404 (`chef-de-chantier.html` inexistant) ; version chef = v1 (sans zone rouge/lycée/paie) ; ⚠️ **prénom dans le sous-domaine ET codé en dur** → recommandé : renommer le site (Site settings → Change site name) et re-déployer avec le dossier `/chef-chantier/` corrigé. NB : `netlify.app` est **bloqué par la politique réseau** des sessions Code (403) — vérifications à faire sur les fichiers locaux.
- **🔒 JAMAIS publier** : le « Mode d'emploi praticienne — Espace Ethan » (reçu le 7 juil) contient prénom + infos cliniques → reste chez Nawel (impression locale), PAS dans le dépôt.
